...
async function apiRequest(apiUrl, token, path, opts = {}) {
  ...
  if (!res.ok) throw new Error(json.error ? JSON.stringify(json.error) : `Request failed (${res.status})`);
  return json;
}
...
// =====================================================================
// Connect screen — login / sign up against a live API, or skip to demo
// =====================================================================
...
function ConnectScreen({ onConnected, onDemo }) {
  ...
  const [email, setEmail] = useState("demo@chaser.app");
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      const json = await apiRequest(apiUrl, null, path, { method: "POST", body: JSON.stringify(body) });
      onConnected({ apiUrl, token: json.token, user: json.user });
    } catch (err) {
      setError(
        err.message?.includes("fetch") || err.message?.includes("Failed")
          ? `Couldn't reach ${apiUrl}. Is the backend running there?`
          : err.message
      );
    } finally {
      setBusy(false);
    ...
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: 600, background: "#0F1B2D", borderRadius: 12, border: "1px solid #1E2E46" }}>
      <div style={{ width: 360, padding: "36px 32px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 22 }}>
          ...
          <span style={{ fontFamily: "'Fraunces', serif", fontWeight: 700, fontSize: 19, color: "#EDEAE0" }}>Chaser</span>
        </div>
        ...
        <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <Field label="API base URL" value={apiUrl} onChange={setApiUrl} placeholder="https://your-backend.up.railway.app" />
          {mode === "signup" && (
            <>
        ...
        <p style={{ fontSize: 11, color: "#5A6B84", marginTop: 16, lineHeight: 1.5 }}>
          Seeded demo login: <code style={{ color: "#8B9AAE" }}>demo@chaser.app</code> / <code style={{ color: "#8B9AAE" }}>password123</code> — once your backend is deployed and reachable at the URL above.
        </p>
      </div>
    </div>
  );
...
function CustomersView({ apiUrl, token, mode }) {
  ...
  if (loading) return <CenterState icon={Loader2} text="Loading customers..." />;
  if (!customers.length) return <CenterState icon={Users} text="No customers yet" />;
    ...
    <div style={{ ...shellStyle, display: "flex" }}>
      ...
      <div style={{ width: 210, background: "#0B1524", borderRight: "1px solid #1E2E46", padding: "20px 14px", display: "flex", flexDirection: "column" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 9, padding: "0 8px", marginBottom: 22 }}>
          ...
          <span style={{ fontFamily: "'Fraunces', serif", fontWeight: 700, fontSize: 17 }}>Chaser</span>
        </div>
      ...
      <div style={{ flex: 1, padding: "22px 28px", overflowY: "auto", maxHeight: 720 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <div>
            <div style={{ fontSize: 11, color: "#5A6B84", fontFamily: "'IBM Plex Mono', monospace", marginBottom: 3 }}>
              {session.apiUrl || "DEMO MODE"}
            </div>
        ...
        {mode === "demo" && showBanner && (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "linear-gradient(90deg, #2A2A1B, #16243A)", border: "1px solid #544A2A", borderRadius: 10, padding: "12px 18px", marginBottom: 20 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 13 }}>
              ...
              You're viewing demo data. Log out and connect your deployed backend URL to see live numbers.
            </div>
