...
import {
  ...
  Zap, ShieldAlert, MessageCircle, MoreVertical,
} from "lucide-react";
...
const STAMP = {
  ...
  note: { label: "NOTE", color: "#8B9AAE", icon: MessageCircle },
};
...
function ModalTextarea({ label, ...props }) {
  return (
    <label style={{ display: "flex", flexDirection: "column", gap: 5, marginBottom: 12 }}>
      <span style={{ fontSize: 11, color: "#8B9AAE", fontWeight: 600 }}>{label}</span>
      <textarea
        ...
        style={{
          background: "#0B1524", border: "1px solid #24354F", borderRadius: 6, padding: "9px 10px",
          color: "#EDEAE0", fontSize: 13, fontFamily: "'Inter', sans-serif", outline: "none", resize: "vertical",
        }}
      />
    </label>
  );
...
function ConfirmModal({ title, message, confirmLabel = "Confirm", danger, busy, error, onConfirm, onClose }) {
  return (
    <Modal title={title} onClose={onClose}>
      <p style={{ fontSize: 13, color: "#8B9AAE", lineHeight: 1.5, marginBottom: 16 }}>{message}</p>
      {error && (
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <button onClick={onConfirm} disabled={busy} style={{
          flex: 1, background: danger ? "#E85D4D" : "#3FB78A", border: "none", color: danger ? "#fff" : "#0B1524",
          fontWeight: 700, fontSize: 13, padding: "9px 0", borderRadius: 7, cursor: "pointer",
        }}>
          {busy ? "Working..." : confirmLabel}
        </button>
        <button onClick={onClose} style={{ background: "transparent", border: "1px solid #24354F", color: "#8B9AAE", fontSize: 13, padding: "9px 14px", borderRadius: 7, cursor: "pointer" }}>
          Cancel
        </button>
      </div>
    </Modal>
...
function RecordPaymentModal({ apiUrl, token, invoice, onClose, onDone }) {
  ...
  const remainingCents = invoice.amountCents - invoice.amountPaidCents;

  const submit = async (e) => {
    e.preventDefault();
    setBusy(true); setError("");
    try {
      await apiRequest(apiUrl, token, `/api/invoices/${invoice.id}/record-payment`, {
        method: "POST",
        body: JSON.stringify({ amountCents: Math.round(parseFloat(amount) * 100), receivedAt, note: note || undefined }),
      });
      onDone();
      onClose();
    } catch (err) {
      setError(err.message || "Couldn't record payment");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <Modal title={`Record payment — ${invoice.number}`} onClose={onClose}>
      <div style={{ fontSize: 12, color: "#8B9AAE", marginBottom: 14 }}>
        Remaining balance: <strong style={{ color: "#EDEAE0" }}>${(remainingCents / 100).toFixed(2)}</strong>
      </div>
      <form onSubmit={submit}>
        <ModalField label="Amount received (USD)" type="number" step="0.01" min="0.01" placeholder={(remainingCents / 100).toFixed(2)} value={amount} onChange={(e) => setAmount(e.target.value)} required />
        <ModalField label="Date received" type="date" value={receivedAt} onChange={(e) => setReceivedAt(e.target.value)} required />
        <ModalField label="Note (optional)" placeholder="e.g. wire transfer, check #1234" value={note} onChange={(e) => setNote(e.target.value)} />
        {error && (
          <div style={{ fontSize: 12, color: "#E85D4D", background: "#E85D4D14", border: "1px solid #E85D4D33", borderRadius: 6, padding: "8px 10px", marginBottom: 12 }}>
            {error}
          </div>
...
function AddNoteModal({ apiUrl, token, invoice, onClose, onDone }) {
  const [note, setNote] = useState("");
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  ...
  const submit = async (e) => {
    ...
    setBusy(true); setError("");
    try {
      await apiRequest(apiUrl, token, `/api/invoices/${invoice.id}/note`, { method: "POST", body: JSON.stringify({ note }) });
      onDone();
      onClose();
    } catch (err) {
      setError(err.message || "Couldn't add note");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <Modal title={`Add note — ${invoice.number}`} onClose={onClose}>
      <form onSubmit={submit}>
        <ModalTextarea label="What happened?" placeholder="e.g. Called AP, they said check is in the mail" value={note} onChange={(e) => setNote(e.target.value)} required />
        {error && (
          <div style={{ fontSize: 12, color: "#E85D4D", background: "#E85D4D14", border: "1px solid #E85D4D33", borderRadius: 6, padding: "8px 10px", marginBottom: 12 }}>
            {error}
          </div>
...
function EditInvoiceModal({ apiUrl, token, invoice, onClose, onDone }) {
  ...
  const submit = async (e) => {
    ...
    setBusy(true); setError("");
    try {
      await apiRequest(apiUrl, token, `/api/invoices/${invoice.id}`, {
        method: "PATCH",
        body: JSON.stringify({ number, amountCents: Math.round(parseFloat(amount) * 100), dueDate }),
      });
      onDone();
      onClose();
    } catch (err) {
  ...
  return (
    <Modal title="Edit invoice" onClose={onClose}>
      <form onSubmit={submit}>
        <ModalField label="Invoice number" value={number} onChange={(e) => setNumber(e.target.value)} required />
        <ModalField label="Amount (USD)" type="number" step="0.01" min="0.01" value={amount} onChange={(e) => setAmount(e.target.value)} required />
    ...
    <Panel>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 10 }}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          {["all", "reminder", "matched", "escalated", "paid", "note", "disputed"].map((f) => (
...
function InvoicesView({ apiUrl, token, mode }) {
  ...
  const [activeModal, setActiveModal] = useState(null); // { type: 'payment'|'note'|'edit'|'dispute'|'delete', invoice }
  ...
  const closeModal = () => { setActiveModal(null); setConfirmError(""); };

  const confirmDispute = async () => {
    if (!activeModal) return;
    setConfirmBusy(true); setConfirmError("");
    try {
      await apiRequest(apiUrl, token, `/api/invoices/${activeModal.invoice.id}/dispute`, { method: "POST", body: JSON.stringify({}) });
      load();
      closeModal();
    } catch (err) {
      setConfirmError(err.message || "Couldn't mark as disputed");
    } finally {
  ...
  const confirmDelete = async () => {
    ...
    setConfirmBusy(true); setConfirmError("");
    try {
      await apiRequest(apiUrl, token, `/api/invoices/${activeModal.invoice.id}`, { method: "DELETE" });
      load();
      closeModal();
    } catch (err) {
      setConfirmError(err.message || "Couldn't delete invoice");
    } finally {
      setConfirmBusy(false);
    }
  };
  ...
  return (
    <Panel>
            ...
            {invoices.map((inv) => (
              <tr key={inv.id}>
                ...
                <td style={{ ...td, display: "flex", gap: 6, alignItems: "center" }}>
                  ...
                  {mode !== "demo" && (
                    <RowMenu items={[
                      { label: "Record payment", icon: DollarSign, disabled: inv.status === "paid", onClick: () => setActiveModal({ type: "payment", invoice: inv }) },
                      { label: "Add note", icon: MessageCircle, onClick: () => setActiveModal({ type: "note", invoice: inv }) },
                      { label: "Mark disputed", icon: ShieldAlert, disabled: inv.status === "disputed" || inv.status === "paid", onClick: () => setActiveModal({ type: "dispute", invoice: inv }) },
                      { label: "Edit", icon: Settings, onClick: () => setActiveModal({ type: "edit", invoice: inv }) },
                      { label: "Delete", icon: X, danger: true, onClick: () => setActiveModal({ type: "delete", invoice: inv }) },
                    ]} />
                  )}
                </td>
              </tr>
      ...
      {activeModal?.type === "payment" && (
        <RecordPaymentModal apiUrl={apiUrl} token={token} invoice={activeModal.invoice} onClose={closeModal} onDone={load} />
      )}
      {activeModal?.type === "note" && (
        <AddNoteModal apiUrl={apiUrl} token={token} invoice={activeModal.invoice} onClose={closeModal} onDone={load} />
      )}
      {activeModal?.type === "edit" && (
        <EditInvoiceModal apiUrl={apiUrl} token={token} invoice={activeModal.invoice} onClose={closeModal} onDone={load} />
      )}
      {activeModal?.type === "dispute" && (
        <ConfirmModal
          title="Mark as disputed?"
          message={`This marks invoice ${activeModal.invoice.number} as disputed. Reminders won't go out for it until you resolve the dispute.`}
      ...
      {activeModal?.type === "delete" && (
        <ConfirmModal
          ...
          message={`This permanently deletes invoice ${activeModal.invoice.number}. This can't be undone.`}
          confirmLabel="Delete"
          danger
          busy={confirmBusy}
          error={confirmError}
  ...
  return (
    <div ref={ref} style={{ position: "relative" }}>
      <button onClick={() => setOpen((o) => !o)} title="More actions" style={{
        background: "#0F1B2D", border: "1px solid #24354F", color: "#8B9AAE",
        borderRadius: 5, padding: "4px 7px", cursor: "pointer", display: "flex", alignItems: "center",
      }}>
        <MoreVertical size={13} />
      </button>
      {open && (
        <div style={{
          position: "absolute", right: 0, top: "calc(100% + 4px)", background: "#16243A",
          border: "1px solid #24354F", borderRadius: 8, minWidth: 172, zIndex: 20,
          ...
          {items.map((item, i) => (
            <button
              ...
              style={{
                ...
                background: "none", border: "none", color: item.danger ? "#E85D4D" : "#EDEAE0",
                fontSize: 12.5, textAlign: "left", cursor: item.disabled ? "default" : "pointer",
                opacity: item.disabled ? 0.4 : 1, fontFamily: "'Inter', sans-serif",
              }}
            >
...
function CustomersView({ apiUrl, token, mode }) {
  ...
  const confirmDelete = async () => {
    ...
    setConfirmBusy(true); setConfirmError("");
    try {
      await apiRequest(apiUrl, token, `/api/customers/${deleteTarget.id}`, { method: "DELETE" });
      load();
      setDeleteTarget(null);
    } catch (err) {
      setConfirmError(err.message || "Couldn't delete customer");
    } finally {
      setConfirmBusy(false);
    }
  };
    ...
    <div>
      ...
      {loading ? <CenterState icon={Loader2} text="Loading customers..." /> :
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 14 }}>
          {customers.map((c) => (
            <Panel key={c.id} style={{ padding: "16px 18px", position: "relative" }}>
              {mode !== "demo" && (
                <button onClick={() => setDeleteTarget(c)} title="Delete customer" style={{
                  position: "absolute", top: 12, right: 12, background: "none", border: "none",
                  color: "#5A6B84", cursor: "pointer", padding: 2,
                }}>
                  <X size={14} />
                </button>
              )}
              <div style={{ fontSize: 14.5, fontWeight: 700, color: "#EDEAE0", marginBottom: 8, paddingRight: 18 }}>{c.name}</div>
      ...
      {deleteTarget && (
        <ConfirmModal
          ...
          message={
            deleteTarget.openInvoiceCount > 0
              ? `${deleteTarget.name} has ${deleteTarget.openInvoiceCount} open invoice(s). Delete those first before removing the customer.`
          ...
          error={confirmError}
          onConfirm={confirmDelete}
          onClose={() => { setDeleteTarget(null); setConfirmError(""); }}
        />
      )}
    </div>
  );
